...
$(function() {
    ...
    setTimeout(function (){
        $("#slogan").removeClass("hidden").addClass("animated bounceInLeft");
    },1000);

	setTimeout(function (){
		$("#slogan-em").removeClass("no-visible").addClass("animated bounceInDown")
	},1300);

	setTimeout(function (){
		$('#slogan-adv').removeClass("hidden").addClass("animated fadeInUp")
	},1000);
	...
	///////////////////////////
	// Smooth scroll
	$("#nav .main-nav a[href^='#'],.slogan-link,.outline-btn").on('click', function(e) {
		e.preventDefault();
		var hash = this.hash;
		$('html, body').animate({
    ...
    // Baidu Map init
    ShowMap('121.450943,31.250148','广尊科技','上海市普陀区江宁路1158号友力国际大厦2702室','021-80126312','','18');
...
function ShowMap(zuobiao, name, address, phone, chuanzhen, zoom) {
    ...
    var infoWindow = new BMap.InfoWindow('<p style="color: #bf0008;font-size:20px;">' + name + '</p><p>地址：' + address + '</p><p>电话：' + phone + '</p>');
...
function getCityList(){
	$.post("/homeset/city/listCity.json",{IP:'127.0.0.1'},function (response){
        var tmp = '';
        response.reObj.forEach(function (item,index,arr){
            if(index===0){
                incity = item.code;
                tmp +='<option value="'+item.code+'" selected>'+item.city+'</option>';
            }else{
...
function setApplyStatus(status){
    if(status){
        if(!$('#apply').hasClass("applying")){
            applying = true;
            $('#apply').prop("disabled",true).addClass("applying");
        }
    }else{
        if($('#apply').hasClass("applying")){
            applying = false;
            $('#apply').prop("disabled",false).removeClass("applying");
        }
    }
}
...
function clearForm(target){
    $(target).find('input:not(.filter-title):not([type="radio"])').val('');
    $(target).find('.filter-title').val($('select option:first-of-type',target).text());
    $(target).find('select option:first-of-type').prop("selected",true);
    $(target).find('input[type="radio"]:first-of-type').prop("checked",true);
}
